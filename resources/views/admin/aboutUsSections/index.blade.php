@extends('layouts.admin')
@section('content')
@can('about_us_section_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.about-us-sections.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.aboutUsSection.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.aboutUsSection.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-AboutUsSection">
                <thead>
                    <tr>
                        <th width="10">

                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.id') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.title') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.ceo_name') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.ceo_image') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.top_right') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.top_left') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.bottom_right') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.bottom_left') }}
                        </th>
                        <th>
                            {{ trans('cruds.aboutUsSection.fields.lang_code') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($aboutUsSections as $key => $aboutUsSection)
                        <tr data-entry-id="{{ $aboutUsSection->id }}">
                            <td>

                            </td>
                            <td>
                                {{ $aboutUsSection->id ?? '' }}
                            </td>
                            <td>
                                {{ $aboutUsSection->title ?? '' }}
                            </td>
                            <td>
                                {{ $aboutUsSection->ceo_name ?? '' }}
                            </td>
                            <td>
                                @if($aboutUsSection->ceo_image)
                                    <a href="{{ $aboutUsSection->ceo_image->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $aboutUsSection->ceo_image->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @if($aboutUsSection->top_right)
                                    <a href="{{ $aboutUsSection->top_right->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $aboutUsSection->top_right->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @if($aboutUsSection->top_left)
                                    <a href="{{ $aboutUsSection->top_left->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $aboutUsSection->top_left->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @if($aboutUsSection->bottom_right)
                                    <a href="{{ $aboutUsSection->bottom_right->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $aboutUsSection->bottom_right->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                @if($aboutUsSection->bottom_left)
                                    <a href="{{ $aboutUsSection->bottom_left->getUrl() }}" target="_blank" style="display: inline-block">
                                        <img src="{{ $aboutUsSection->bottom_left->getUrl('thumb') }}">
                                    </a>
                                @endif
                            </td>
                            <td>
                                {{ App\Models\AboutUsSection::LANG_CODE_SELECT[$aboutUsSection->lang_code] ?? '' }}
                            </td>
                            <td>
                                @can('about_us_section_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.about-us-sections.show', $aboutUsSection->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('about_us_section_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.about-us-sections.edit', $aboutUsSection->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('about_us_section_delete')
                                    <form action="{{ route('admin.about-us-sections.destroy', $aboutUsSection->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                    </form>
                                @endcan

                            </td>

                        </tr>
                    @endforeach
                </tbody>
            </table>
        </div>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('about_us_section_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.about-us-sections.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  });
  let table = $('.datatable-AboutUsSection:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection