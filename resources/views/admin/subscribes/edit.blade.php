    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.subscribes.update", [$subscribe->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="email">{{ trans('cruds.subscribe.fields.email') }}</label>
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $subscribe->email) }}" required>
                @if($errors->has('email'))
